...
public class RecordFBOActivity extends Activity
  ...
  private static class RenderThread extends Thread {
    ...
    private void doFrame(long timeStampNanos) {
      ...
      if (!mRecordingEnabled || mRecordedPrevious) {
        ...
        if (mRecordMethod == RECMETHOD_DRAW_TWICE) {
          ...
          GLES20.glViewport(mVideoRect.left, mVideoRect.top, mVideoRect.width(),
              mVideoRect.height());
          GLES20.glEnable(GLES20.GL_SCISSOR_TEST);
          GLES20.glScissor(mVideoRect.left, mVideoRect.top, mVideoRect.width(),
              mVideoRect.height());
          draw();
          ...
          Log.v(TAG, "glBlitFramebuffer: 0,0,"
    ...
    private void update(long timeStampNanos) {
      ...
      if ((mRectVelX < 0 && xpos - xscale / 2 < mInnerLeft) || (mRectVelX > 0
          && xpos + xscale / 2 > mInnerRight + 1)) {
        mRectVelX = -mRectVelX;
      }
      if ((mRectVelY < 0 && ypos - yscale / 2 < mInnerBottom) || (mRectVelY > 0
          && ypos + yscale / 2 > mInnerTop + 1)) {
        mRectVelY = -mRectVelY;
      }
